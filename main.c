...
void random_shuffle_gene(gene* l,int size){
    int i,r1,r2;
     gene t;
    for(i=0;i<GENOME_SIZE*2;i++){
        r1=rand()%size;
...
void print_genome(gene* g){
    int i;
    for(i=0;i<GENOME_SIZE;i++){
        printf("%c%c%c %2d %.2f %.2f %d",g[i].use,g[i].b_dir,g[i].r_dir,g[i].rel,g[i].width,g[i].height,g[i].ensuite);
    }
}

//prints n genomes randomly sampled from p
void print_sample(gene*** p,int n){
    int index;
    for(;n>=0;n--){
...
int generate_score_array(gene*** p,double* s){
    int i;
...
void crossover(gene*** p,gene*** new_population,int* unique_randoms,double* s,double tot){
...
int main(){
    ...
    int i;
     ...
     gene** population;
    population=( gene**)malloc(POPULATION_SIZE*sizeof( gene*));
    for(i=0;i<POPULATION_SIZE;i++){
        population[i]=( gene*)malloc(GENOME_SIZE*sizeof( gene));
    }

     gene** new_population;
    new_population=( gene**)malloc(POPULATION_SIZE*sizeof( gene*));
    for(i=0;i<POPULATION_SIZE;i++){
        new_population[i]=( gene*)malloc(GENOME_SIZE*sizeof( gene));
    }
